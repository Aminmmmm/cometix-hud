...
# ============================================================
# cometix-hud 卸载脚本
# https://github.com/Aminmmmm/cometix-hud
# 
# 使用方式：
#   ./uninstall-cometix-hud.sh        # 交互式
#   ./uninstall-cometix-hud.sh -y     # 跳过确认
#   curl -fsSL .../uninstall-cometix-hud.sh | bash -s -- -y
# ============================================================
...
echo ""
echo "=========================================="
echo "  cometix-hud 卸载工具"
echo "=========================================="
echo ""

# 确认卸载（支持 -y 参数跳过）
if [[ "$1" != "-y" ]]; then
    read -p "确认卸载 cometix-hud？(y/N): " confirm < /dev/tty || confirm="n"
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        echo "已取消卸载"
        exit 0
    fi
...
# 4. 清理临时文件
info "正在清理临时文件..."
rm -rf /tmp/cometix-hud 2>/dev/null && success "已删除 /tmp/cometix-hud"
rm -rf /tmp/claude-hud-fusion 2>/dev/null && success "已删除 /tmp/claude-hud-fusion"
...
try:
    ...
    if 'cometix-hud' in plugins:
        del plugins['cometix-hud']
        with open('$REGISTRY_FILE', 'w') as f:
            json.dump(registry, f, indent=2)
        print('已从注册表移除 cometix-hud')
except:
    pass
...
echo ""
echo "=========================================="
echo -e "  ${GREEN}cometix-hud 卸载完成！${NC}"
echo "=========================================="
echo ""
