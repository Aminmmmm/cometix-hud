...
export type LineLayoutType = 'compact' | 'expanded' | 'powerline' | 'auto';
...
function validateLineLayout(value: unknown): value is LineLayoutType {
  return value === 'compact' || value === 'expanded' || value === 'powerline' || value === 'auto';
}
...
export function mergeConfig(userConfig: Partial<HudConfig>): HudConfig {
  ...
  const theme = typeof migrated.theme === 'string' && migrated.theme.trim()
    ? migrated.theme.trim()
  ...
  return { language, lineLayout, theme, showSeparators, pathLevels, maxWidth, forceMaxWidth, elementOrder, gitStatus, display, colors };
}
