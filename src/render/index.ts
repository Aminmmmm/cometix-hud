...
import { renderPowerline } from './powerline.js';
...
export function render(ctx: RenderContext): void {
  let lineLayout = ctx.config?.lineLayout ?? 'expanded';

  // Auto mode: powerline when terminal is wide enough (≥120 cols), expanded otherwise
  if (lineLayout === 'auto') {
    const detectedWidth = getTerminalWidth({ preferEnv: true, fallback: UNKNOWN_TERMINAL_WIDTH });
    lineLayout = (detectedWidth !== UNKNOWN_TERMINAL_WIDTH && detectedWidth >= 120) ? 'powerline' : 'expanded';
  }

  // Powerline mode: single-line CCometixLine-style renderer
  if (lineLayout === 'powerline') {
    renderPowerline(ctx);
    return;
  }
