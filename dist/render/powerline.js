...
const DIM = '\x1b[2m';
const BOLD = '\x1b[1m';
function iconize(theme, key, text) {
    const icon = theme.icons[key];
    return icon ? `${icon} ${text}` : text;
}
function segment(theme, color, iconKey, text) {
...
function contextBar(theme, percent, width = 10) {
    const filled = Math.round((percent / 100) * width);
    const empty = width - filled;
    const remaining = 100 - percent;
    let color;
    if (remaining < 20)
        color = theme.contextCritical;
    else if (remaining < 50)
        color = theme.contextWarn;
    else
        color = theme.context;
    return `${color}${theme.barFilled.repeat(filled)}${DIM}${theme.barEmpty.repeat(empty)}${RESET}`;
}
/** Usage bar */
function usageBar(theme, percent, width = 10) {
    const filled = Math.round((percent / 100) * width);
    const empty = width - filled;
    let color;
    if (percent >= 90)
...
function gitStatusIcon(isDirty) {
    return isDirty ? '●' : '✓';
}
export function renderPowerline(ctx) {
    ...
    if ((gitConfig?.enabled ?? true) && ctx.gitStatus) {
        ...
        if (gitConfig?.showAheadBehind) {
            if (ctx.gitStatus.ahead > 0)
                gitText += ` ${theme.git}\u2191${ctx.gitStatus.ahead}${RESET}`;
            if (ctx.gitStatus.behind > 0)
                gitText += ` ${theme.git}\u2193${ctx.gitStatus.behind}${RESET}`;
        }
        if (gitConfig?.showFileStats && ctx.gitStatus.fileStats) {
            ...
            if (statParts.length > 0)
                gitText += ` ${DIM}${statParts.join(' ')}${RESET}`;
        }
